...
// Created once per process and handed to the writer, as the event writer does it.
func NewMetrics(registry *metrics.Registry) *Metrics {
	instruments := &Metrics{
		events: prometheus.NewCounterVec(prometheus.CounterOpts{
			Namespace: metrics.Namespace,
			Subsystem: "detectionstore",
			Name:      "detections_total",
			Help:      "Records by what the detection writer did with them.",
		}, []string{"outcome"}),
		...
		refusals: prometheus.NewCounterVec(prometheus.CounterOpts{
			...
			Subsystem: "detectionstore",
			Name:      "refusals_total",
			Help:      "Records that were not detections this build could store, by why.",
		}, []string{"reason"}),
		batch: prometheus.NewHistogram(prometheus.HistogramOpts{
			Namespace: metrics.Namespace,
			Subsystem: "detectionstore",
			Name:      "batch_detections",
			Help:      "Records carried by a batch handed to the detection writer.",
		...
		write: prometheus.NewHistogram(prometheus.HistogramOpts{
			...
			Help:      "Time spent making a batch of detections durable in the store.",
...
// distinct detections. `batches_total{outcome="retried"}` explains the difference.
func (m *Metrics) stored(detections int, elapsed time.Duration) {
	m.events.WithLabelValues("stored").Add(float64(detections))
