...
// is what keeps `internal/advisorystore` from learning what a Kafka offset is.

type source struct{ consumer *broker.Consumer }

func (s source) Consume(ctx context.Context, deliver advisorystore.Deliver) error {
	return s.consumer.Consume(ctx, func(ctx context.Context, records []broker.Record) error {
		return deliver(ctx, fetched(records))
	})
}

func fetched(records []broker.Record) []advisorystore.Record {
	converted := make([]advisorystore.Record, 0, len(records))
	for _, record := range records {
		converted = append(converted, advisorystore.Record{
...
func (q quarantine) Publish(ctx context.Context, refused []advisorystore.Refused) error {
	return q.topic.Publish(ctx, rejected(refused))
}

func rejected(refused []advisorystore.Refused) []broker.Refused {
