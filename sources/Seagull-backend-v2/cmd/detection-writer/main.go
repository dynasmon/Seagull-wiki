...
func main() {
	...
	if err := writer(ctx); err != nil {
		fmt.Fprintf(os.Stderr, "detection-writer: %v\n", err)
		os.Exit(1)
	}
}

// The consumer that makes a detection queryable. It reads what the analysis
// engine decided and knows nothing about how it was decided: no rule, no
// ruleset, no engine. The two meet on the backbone and nowhere else.
func writer(ctx context.Context) error {
	...
	store, err := clickhouse.NewDetectionStore(settings.store)
	if err != nil {
		return err
	}
	...
	drift, err := consumer.VerifyTopics(topologyCtx, settings.topology.Detections, settings.topology.DetectionsQuarantine)
	if err != nil {
		return err
	}
	...
	refused, err := broker.NewQuarantine(broker.Config{
		Brokers:  settings.brokers,
		Topic:    settings.topology.DetectionsQuarantine.Name,
		ClientID: serviceName,
		Security: settings.security,
	})
	if err != nil {
	...
	component, err := detectionstore.NewWriter(detectionstore.WriterOptions{
		Source:        source{consumer: consumer},
		Sink:          store,
		Quarantine:    quarantine{topic: refused},
		Metrics:       detectionstore.NewMetrics(platform.Metrics()),
	...
	platform.Health().Register("detection-store", store.Ping)
	platform.Health().Register("backbone", consumer.Ping)
	platform.Add(component)

	platform.Logger().Info("detection_writer_configured",
		slog.String("topic", settings.topology.Detections.Name),
		slog.String("quarantine_topic", settings.topology.DetectionsQuarantine.Name),
