...
// is what keeps `internal/inventorystore` from learning what a Kafka offset is.

type source struct{ consumer *broker.Consumer }

func (s source) Consume(ctx context.Context, deliver inventorystore.Deliver) error {
	return s.consumer.Consume(ctx, func(ctx context.Context, records []broker.Record) error {
		return deliver(ctx, fetched(records))
	})
}

func fetched(records []broker.Record) []inventorystore.Record {
...
func (q quarantine) Publish(ctx context.Context, refused []inventorystore.Refused) error {
	return q.topic.Publish(ctx, rejected(refused))
}

func rejected(refused []inventorystore.Refused) []broker.Refused {
