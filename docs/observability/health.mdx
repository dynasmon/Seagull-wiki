...
Today the agent emits structured startup, privilege, core-dump, configuration, and stop diagnostics. Spool depth, upload failures, collector throughput, and heartbeat monitoring remain target signals until those components exist. The current executable has no metrics endpoint; see [agent diagnostics](/docs/agent/diagnostics).
