...
Unknown protobuf fields may be ignored, but that does not permit a silent change to what a response means. A new interpretation that an older client cannot safely make requires an explicit protocol transition. The agent's recorded backend exchanges limit compatibility claims to the platforms they were recorded from; they do not establish compatibility with every version bearing a similar schema.
